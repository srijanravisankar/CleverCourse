...
interface GamificationState {
  ...
  triggerXpAnimation: (amount: number, bonusAmount: number, reason: string) => void;
...
export const useGamificationStore = create<GamificationState>((set, get) => ({
  ...
  processXpResult: (result: AwardXpResult) => {
    const { triggerXpAnimation, triggerLevelUp, addAchievementToQueue, setStats, stats } = get();
    ...
    if (stats) {
      setStats({
        ...
        xpProgress: Math.min(100, Math.floor(
          ((result.newTotal - result.xpForCurrentLevel) / 
          (result.xpForNextLevel - result.xpForCurrentLevel)) * 100
        )),
