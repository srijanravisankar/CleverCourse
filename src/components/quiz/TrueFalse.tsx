...
export function TrueFalse({ 
  ...
  const handleSubmit = () => {
    if (userChoice !== null) setIsSubmitted(true)
  }

  const isCorrect = userChoice === answer

  return (
    <Card className="w-full max-w-xl mx-auto border-none shadow-xl bg-white overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <CardContent className="pt-10 pb-10 space-y-6 px-10">
        <div className="flex gap-4">
          ...
          >
            ...
            {isSubmitted && answer === true && <Check className="mt-2 size-6" />}
            {isSubmitted && userChoice === true && answer === false && <X className="mt-2 size-6" />}
          </button>
          ...
          >
            ...
            {isSubmitted && answer === false && <Check className="mt-2 size-6" />}
            {isSubmitted && userChoice === false && answer === true && <X className="mt-2 size-6" />}
          </button>
        </div>

        {isSubmitted && (
          <div className="mt-8 p-6 rounded-2xl bg-slate-900 text-slate-100 animate-in zoom-in-95 duration-300 shadow-2xl">
            <div className="flex items-center gap-2 mb-2">
              <div className={`size-2 rounded-full ${isCorrect ? "bg-green-400" : "bg-red-400"}`} />
      ...
      <CardFooter className="bg-slate-50/50 border-t p-6 flex justify-between">
        ...
        {!isSubmitted ? (
          ...
          <Button onClick={onNext} className="gap-2 px-10 bg-green-600 hover:bg-green-700 text-white rounded-full font-bold shadow-lg shadow-green-200">
            Continue
