...
import { seedAchievements, checkAchievementsSeeded } from '@/app/actions/seed-gamification';
...
export function GamificationProvider({ children }: GamificationProviderProps) {
  ...
  const [currentAchievement, setCurrentAchievement] = useState<typeof achievementQueue[0] | null>(null);
  ...
  return (
    <>
      ...
      <LevelUpModal
        ...
        xpToNext={levelUpData ? getXpForNextLevel(levelUpData.newLevel) - (stats?.xpTotal ?? 0) : 0}
      />
      ...
      {showFreezeUsedNotification && (
        ...
        >
          <span className="text-cyan-400">❄️ Streak freeze used! Your streak is protected.</span>
        </div>
      )}
    </>
  );
